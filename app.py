...
HOURS_BACK   = 24
MAX_ARTICLES = 60   # Limite articles envoyés au LLM
...
- Cite un maximum d'actualités différentes parmi celles fournies.
...
def inject_css(accent: str = "#c0392b"):
    ...
    @media (min-width: 769px) {{
        .block-container {{ 
            max-width: 900px !important; /* Interface plus large sur PC */
        }}
        .masthead-title {{ 
            font-size: 4rem; 
        }}
        .summary-card {{ 
            padding: 2.5rem 3rem; /* Marges confortables comme un livre */
            font-size: 1.05rem; 
            line-height: 1.8;
        }}
        /* Style des titres pour séparer visuellement les gros blocs de texte */
        .summary-card h2 {{
    ...
    @media (max-width: 768px) {{
        ...
        .stButton > button {{
            padding: 1rem !important; /* Bouton plus grand pour les doigts */
...
def _md_to_html(text: str) -> str:
    """Conversion Markdown→HTML améliorée pour les textes longs."""
    # Titres ## → <h2>
    ...
    text = text.replace("\n\n", "<br><br>")
    # Conserver les listes à puces simples
